...
@property (nonatomic, strong) NSMutableArray <KSYMETimeLineItem*> *timelineItems;
...
@implementation KSYTimelineView
{
    BOOL _isDragging;
    BOOL _isDecelerate;
...
- (NSArray *)checkPasterExistBetween:(CGFloat)beginTime and:(CGFloat)endTime
{
    ...
    for (KSYMETimeLineItem *item in self.timelineItems) {
        if (beginTime >= item.startTime && item.endTime >= beginTime && item.endTime <= endTime) {
            KSYTimelinePercent *timelinePercent = [[KSYTimelinePercent alloc] init];
...
- (void)setupSubviews
{
    ...
    [self.collectionView registerClass:[KSYTimelineItemCell class] forCellWithReuseIdentifier:@"KSYTimelineItemCell"];
...
{
    __block KSYMETimeLineItem *targetItem = nil;
    
    [self.timelineItems enumerateObjectsUsingBlock:^(KSYMETimeLineItem *item, NSUInteger idx, BOOL * _Nonnull stop) {
...
{
    KSYTimelineItemCell *timelineCell = [collectionView dequeueReusableCellWithReuseIdentifier:@"KSYTimelineItemCell" forIndexPath:indexPath];
    ...
    [timelineCell setMappedBeginTime:mappedBeginTime endTime:mappedEndTime image:image timelinePercents:timelinePercent];
