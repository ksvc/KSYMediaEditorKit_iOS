...
@property (weak, nonatomic) IBOutlet UISegmentedControl *seBlendModeSeg;
...
- (void)configSubviews{
    ...
    [self.filterEffectCollectionView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self).insets(UIEdgeInsetsMake(40, 5, 5, 5));
    }];
    
    [self.seBlendModeSeg mas_makeConstraints:^(MASConstraintMaker *make) {
...
#pragma mark - Blend Mode Seg Ctrl
- (IBAction)didChangeBlendMode:(UISegmentedControl *)sender {
    if ([self.delegate respondsToSelector:@selector(didChangeFilterEffectBlendMode:)]){
        [self.delegate didChangeFilterEffectBlendMode:(KSYSEBlendMode)sender.selectedSegmentIndex];
    }
}
