...
@interface KSYEditStickerCell () <UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation KSYEditStickerCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)configSubviews{
    ...
    self.stickerCollectionView.collectionViewLayout = layout;
    [self.stickerCollectionView registerNib:[UINib nibWithNibName:[KSYEditStickerCollectionViewCell className] bundle:[NSBundle mainBundle]] forCellWithReuseIdentifier:[KSYEditStickerCollectionViewCell className]];
    
    [self.stickerCollectionView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)setupModels{
    ...
    for (int i = 0; i < 8; i++) {
        NSString *str = [NSString stringWithFormat:@"decal_%d_icon",i];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if ([self.delegate respondsToSelector:@selector(editPanelStickerType:selectedIndex:)]) {
        [self.delegate editPanelStickerType:KSYMEEditStickerTypeSticker selectedIndex:indexPath.row];
    }
}
