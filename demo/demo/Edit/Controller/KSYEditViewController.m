...
- (void)resizePreviewBGViewWithResizeMode:(KSYMEResizeMode)mode Ratio:(KSYMEResizeRatio)ratio{
    ...
    if (mode == KSYMEResizeModeFill) {
        _previewBGView.scrollEnabled = NO;
    } else {
        _previewBGView.scrollEnabled = YES;
    }
    // 1. 分辨率
...
- (void)editPanelView:(KSYEditPanelView *)view songFilePath:(NSString *)filePath{
    ...
    [_editor pausePreview];
    [_editor seekToTime:_editor.videoRange.start range:kCMTimeRangeInvalid finish:nil];
...
- (void)didChangeFilterEffectBlendMode:(KSYSEBlendMode)mode {
    [_editor setSeMode:mode];
    [_editor resumePreview];
    _playBtn.hidden = YES;
}
