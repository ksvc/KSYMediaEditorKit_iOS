            ...
            appKey:(NSString *)appKey
{
    if (self = [super init]) {
        ...
        _textureOutput.bCustomOutputSize = YES;
        _textureOutput.outputSize = CGSizeMake(360, 640);
...
- (void)downLoadMetarials{
    ...
    [self.service fetchMaterialsWithGroupID:@"144a71510ef645c797f41ea23d8fc6e9" onSuccess:^(NSArray<SenseArMaterial *> *arrMaterials) {
...
{
    //资源不存在，下载
    [self.service downloadMaterial:material onSuccess:completeSuccess onFailure:completeFailure onProgress:processingCallBack];
}
- (BOOL)isDownloadComplete:(SenseArMaterial *)stickers
{
    return [self.service isMaterialDownloaded:stickers.strID];
}
